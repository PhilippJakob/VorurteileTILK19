...
{	
	...
	public static ArrayList<Fakt> suchenUntergeordneteFakten(int pVorurteilID)
	{
		...
		try
		{
			Connection lVerbindung = Datenbankverbindung.getConnection();
			Statement lBefehl = lVerbindung.createStatement();
			ResultSet lErgebnis = lBefehl.executeQuery("SELECT f.Titel, f.ID_Fakten, f.Autor, f.Veröffentlichung, f.InternetQuelle_Ja_Nein, f.Link, f.Datum_Stempel, f.Aussage FROM dbo_vorurteile.fakten f, dbo_vorurteile.verbindung_f_v ve, dbo_vorurteile.vorurteile vo WHERE vo.ID_Vorurteile = ve.ID_Vorurteile AND ve.ID_Fakten = f.ID_Fakten AND vo.ID_Vorurteile=" + pVorurteilID + ";");
			while(lErgebnis.next())
			{
				...
				lListeFakten.add(new Fakt(lErgebnis.getString("f.Titel"), lErgebnis.getString("f.Autor"),
							...
							lErgebnis.getString("f.InternetQuelle_Ja_Nein"),lErgebnis.getString("f.Link"),
